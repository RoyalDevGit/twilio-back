...
const EMAIL_SESSION_FAILED_PAYMENT_CANCELLATION_CONSUMER_TEMPLATE_ID =
  Env.getString(
    'EMAIL_SESSION_FAILED_PAYMENT_CANCELLATION_CONSUMER_TEMPLATE_ID'
  )

export const FailedPaymentSessionCancelledConsumerConfig =
  new NotificationConfig({
    id: NotificationType.SessionFailedPaymentCancellation,
    audience: NotificationAudience.Consumer,
    allowOptOut: true,
    templates: [
      {
        subjectTemplateKey: 'failedPaymentSessionCancellation.consumer.subject',
      ...
      {
        bodyTemplateKey:
          'failedPaymentSessionCancellation.consumer.plainText.body',
...
const queueConsumerNotification = async ({
  ...
  await queueNotification({
    currentUser,
    targetUser: consumer,
    config: FailedPaymentSessionCancelledConsumerConfig,
    payload,
  })
}

const EMAIL_SESSION_FAILED_PAYMENT_CANCELLATION_EXPERT_TEMPLATE_ID =
  Env.getString('EMAIL_SESSION_FAILED_PAYMENT_CANCELLATION_EXPERT_TEMPLATE_ID')

export const FailedPaymentSessionCancelledExpertConfig = new NotificationConfig(
  {
    id: NotificationType.SessionFailedPaymentCancellation,
    ...
    templates: [
      {
        subjectTemplateKey: 'failedPaymentSessionCancellation.expert.subject',
        contentFormat: NotificationContentFormat.HTML,
        channels: [NotificationChannel.Email],
        externalBodyTemplateId:
          EMAIL_SESSION_FAILED_PAYMENT_CANCELLATION_EXPERT_TEMPLATE_ID,
      },
      {
        bodyTemplateKey:
          'failedPaymentSessionCancellation.expert.plainText.body',
        contentFormat: NotificationContentFormat.PlainText,
        channels: [
          NotificationChannel.NotificationTray,
          NotificationChannel.PushNotification,
...
const queueExpertNotification = async ({
  ...
  await queueNotification({
    ...
    config: FailedPaymentSessionCancelledExpertConfig,
    payload,
  })
}

export const queueFailedPaymentSessionCancellationNotifications = async (
